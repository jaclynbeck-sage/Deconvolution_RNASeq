...
map_folder <- Folder("map_reference", parent = "syn49332774")
...
# Helper function
uploadFile <- function(filename, parent_folder, dataset) {
...
# Processed single cell and bulk datasets
for (dataset in datasets) {
  files <- list.files(dir_input, pattern = dataset, full.names = TRUE)
  for (filename in files) {
    uploadFile(filename, input_folder, dataset)
  }
}
...
for (filename in files) {
  uploadFile(filename, map_folder, "cain")
}
