...
#                   "log_cpm" will take the log2(cpm+0.5)
#                   "log_tpm" will take the log2(tpm+0.5)
#                   "log_tmm" will take the log2(tmm+0.5)
...
Load_CountsFile <- function(filename, output_type, regression_method = "none") {
  ...
  if (grepl("log", output_type)) {
    if (is(norm_counts, "matrix")) {
      norm_counts <- log2(norm_counts + 0.5)
    }
    else { # Sparse matrix
      norm_counts@x <- log2(norm_counts@x + 0.5)
    }
  }
...
#                 'log', the signature matrix will be transformed as log2(x+0.5).
